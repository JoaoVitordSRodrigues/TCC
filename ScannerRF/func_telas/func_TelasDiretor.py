...
class Config_TelasDiretor(QMainWindow):
    ...
    def cadastrarAluno(self):
        ...
        nomeAluno = self.ui.etyNome_PgCadastro.text()
        rmAluno = self.ui.etyRM_PgCadastro.text()
        periodoAluno = self.ui.cboxPeriodo_PgCadastro.currentText()
        cursoAluno = self.ui.cboxCurso_PgCadastro.currentText()
        serie_turmaAluno = self.ui.cboxTurma_PgCadastro.currentText()
        face_aluno = self.face_aluno
        #importando da classe aluno o metodo de cadastrar
        self.classeAluno.cadastrarAluno(nomeAluno, rmAluno, periodoAluno, cursoAluno, serie_turmaAluno, face_aluno)

        resultado = self.classeAluno.cadastrarAluno(nomeAluno, rmAluno, periodoAluno, cursoAluno, serie_turmaAluno, face_aluno)

        
        #ira fazer a validação dos campos não preechidos
        if(resultado == "Preencha todos os campos!"):
            QMessageBox.warning(self, "Aviso", resultado)#mensagem de campos nao preenchidos
        ...
        elif(resultado == "O nome deve conter apenas letras e espaços."):
            QMessageBox.warning(self, "Aviso", resultado)
            self.ui.etyNome_PgCadastro.setStyleSheet('background-color: #069E6E; border-radius:5px; border: 2px solid red; color:#ffffff')

        #verifica se o RM contém apenas dígitos
        elif(resultado == "O RM deve conter apenas números."):
            QMessageBox.warning(self, "Aviso", resultado)
            self.ui.etyRM_PgCadastro.setStyleSheet('background-color: #069E6E; border-radius:5px; border: 2px solid red; color:#ffffff')

        elif(resultado == "Selecione alguma imagem do aluno. O aluno não contém imagem para cadastrar."):
            QMessageBox.warning(self, "Aviso", resultado)
